...
class T extends Thread{
    ...
    private VetorN vetorN;  //objeto compartilhado entre as threads

    //construtor
    public T(int id, int NTHREADS, int bloco, VetorN vetorN){
        this.id = id;
    ...
    public void run(){
        ...
        if(this.id == NTHREADS-1){
            end = this.vetorN.dimVetor;  //trata o resto
        }else{
            end = (id+1)*(this.bloco);
        }
...
class NumPares {
    ...
    public static void main (String[] args) {
        ...
        VetorN vetorN = new VetorN(vetor);

        //cria as threads da aplicacao
        for (int i = 0; i < threads.length; i++){
            threads[i] = new T(i, NTHREADS, bloco, vetorN);
        }
        ...
        if (nParesS == vetorN.getnParesC()){
            System.out.println("O vetor tem " + vetorN.getnParesC() + " numeros pares.");
        } else {
